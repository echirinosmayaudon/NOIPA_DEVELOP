...
<section class="portlet" id="portlet_${portlet_id}">
	<#--Controllo per verificare che la portlet non sia un webContent e che non smarcata con la variabile del non intestazione--->
		<#assign nonWebContent= (!(portlet_id?contains("journal_content_web"))) && (portlet_display.getPortletDecoratorId() != "portletNonIntestazione") >

	<#--Controllo per verificare che si è un web content essi sia richiesto come intestazione--->
		<#assign isWebContentIntestabile=(portlet_display.getPortletDecoratorId() == "webContentIntestazione") >

	<#---Intestazione dinamica per la accessibiità--->
		<#if nonWebContent || isWebContentIntestabile>
			<h1 class="sr-only">${portlet_title}</h1>
		</#if>
	...
	<div class="${portlet_content_css_class}">
		...
		<#--
		<#if portlet_display.getPortletDecoratorId() != "barebone" && portlet_display.getPortletDecoratorId() != "noipasearch" >
